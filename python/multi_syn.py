...
psr.add_argument('-a', '--add', required=True, help='加減算器の個数')
psr.add_argument('-c', '--clock_period', required=True, help='クロック制約')
args = psr.parse_args()
...
def syn(clk):
    ...
    with open(res_dir+'/syn.log',mode='w') as f:
        proc = subprocess.run('(time dc_shell) > dc_result/'+prex+'/syn.log  2>&1',shell=True,input = template.format(clk=clk,prex=prex,res_dir=res_dir).encode(),stdout=f)

    res = [clk]
    with open(res_dir+'/timing.txt',mode='r') as f:
        for l in f:
            if 'slack' in l and (not 'sort_by' in l):
...
with open("result_add{0}.csv".format(add_num),'a') as f:

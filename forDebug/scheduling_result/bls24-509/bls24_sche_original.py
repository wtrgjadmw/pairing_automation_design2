...
def find_mistake(split_ope, depth, pre_sche_result, output_value):
    ...
    for formula in split_ope[depth]:
        ...
        if formula[2] == formula[3]: # "_mem1" is not exist
            cnt += 1
        if formula[2] == "T4_14cc" or formula[3] == "T4_14cc":
            cnt += 1
...
def m24_conj_solution_change(formulas_m24, formulas_m24_conj, solution_m24_conj):
    for i in range(len(formulas_m24)):
        if formulas_m24[i] == formulas_m24_conj[i]:
            continue
        if formulas_m24[i][1] != formulas_m24_conj[i][1]:
            continue
        if formulas_m24[i][2] == formulas_m24_conj[i][3]:
            for j in range(len(solution_m24_conj)):
                if solution_m24_conj[i][0] == formulas_m24[i][1]+"_mem0":
                    solution_m24_conj[i][0] = formulas_m24[i][1]+"_mem1"
                elif solution_m24_conj[i][0] == formulas_m24[i][1]+"_mem1":
                    solution_m24_conj[i][0] = formulas_m24[i][1]+"_mem0"
        else:
            print(formulas_m24[i], formulas_m24_conj[i])
    return solution_m24_conj
...
if __name__ == "__main__":
    ...
    # formulas, add_formula_num, mul_formula_num = read_formula_csv("/home/mfukuda/optimal-ate-pairing/RTL/BLS24-509/formulas/{0}.csv".format(algo_name))
    formulas, mem_table, add_formula_num, mul_formula_num = read_formula_csv("/home/mfukuda/optimal-ate-pairing/scheduling/formulas/bls24/{0}.csv".format(algo_name))
    ...
    if algo_name == "padd":
        ...
        conj_formulas, conj_mem_table, add_formula_num, mul_formula_num = read_formula_csv("/home/mfukuda/optimal-ate-pairing/scheduling/formulas/bls24/{0}.csv".format("m24_conj"))
        solution_m24_conj = m24_conj_solution_change(formulas_m24=formulas, formulas_m24_conj=conj_formulas, solution_m24_conj=solution)
        conj_filename = "{2}_mul{0}_add{1}".format(mul_num, add_num, "conj")
        write_result(conj_filename, input_value, output_value, solution_m24_conj, conj_formulas, conj_mem_table)

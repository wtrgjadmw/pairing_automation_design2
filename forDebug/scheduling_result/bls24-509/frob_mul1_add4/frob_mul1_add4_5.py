...
def solve_frob_mul1_add4_5(ConstStep, ExpStep):
	horizon = 621
	S=Scenario("frob_mul1_add4_5",horizon = horizon)
	...
	C10t4_t2t3_in += alt(MUL_in)
	C10t4_t2t3 = S.Task('C10t4_t2t3', length=7, delay_cost=1)
	C10t4_t2t3 += alt(MUL)
	S+=C10t4_t2t3>=C10t4_t2t3_in
	...
	C01t4_t2t3_in += alt(MUL_in)
	C01t4_t2t3 = S.Task('C01t4_t2t3', length=7, delay_cost=1)
	C01t4_t2t3 += alt(MUL)
	S+=C01t4_t2t3>=C01t4_t2t3_in
	...
	C11t4_t2t3_in += alt(MUL_in)
	C11t4_t2t3 = S.Task('C11t4_t2t3', length=7, delay_cost=1)
	C11t4_t2t3 += alt(MUL)
	S+=C11t4_t2t3>=C11t4_t2t3_in
	...
	C02t4_t2t3_in += alt(MUL_in)
	C02t4_t2t3 = S.Task('C02t4_t2t3', length=7, delay_cost=1)
	C02t4_t2t3 += alt(MUL)
	S+=C02t4_t2t3>=C02t4_t2t3_in
	...
	C12t4_t2t3_in += alt(MUL_in)
	C12t4_t2t3 = S.Task('C12t4_t2t3', length=7, delay_cost=1)
	C12t4_t2t3 += alt(MUL)
	S+=C12t4_t2t3>=C12t4_t2t3_in
	...
	pic_file_name = "frob_mul1_add4/frob_mul1_add4_5.png"
	if(S.solution() != []):
...
if __name__ == "__main__":
	solution = solve_frob_mul1_add4_5(0, 0)